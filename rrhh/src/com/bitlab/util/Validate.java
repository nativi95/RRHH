...
public class Validate {
    ...
    /**
     * metodo que recibira la captura para posteriormente evaluarla si es u
     * numero entero y retornara el mismo
     *
     * @param user
     ...
     * @return int numero capturado en consola
     */
    public int isNumeric() {// Se repite
        boolean flag = true;
        String fromCmd = null;
        while (flag) {
            fromCmd = s.nextLine();
            if (fromCmd.toLowerCase().equals("cancel")) {
                return 0;
            }
            flag = validateNumber(fromCmd);
        }

        return Integer.parseInt(fromCmd);
